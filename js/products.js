...
function productRow(p, categories) {
  ...
  return `
    <div class="list-item" style="cursor:pointer;${p.is_active === false ? 'opacity:.5;' : ''}" data-edit="${p.id}">
      <div class="avatar">${p.icon || (cat ? cat.icon : '📦')}</div>
      <div class="info">
...
function openProductForm(container, product, onSaved) {
  ...
  const overlay = openSheet(`
    ...
    <div class="form-group">
      <label>رمز تعبيري مخصص للمنتج (اختياري)</label>
      <input type="text" id="pf-icon" value="${isEdit ? escapeHtml(product.icon || '') : ''}" placeholder="اتركه فارغًا لاستخدام رمز التصنيف" style="font-size:22px;text-align:center;" maxlength="4">
    </div>
  ...
  overlay.querySelector('#pf-save').onclick = async () => {
    ...
    const data = {
      ...
      icon: overlay.querySelector('#pf-icon').value.trim(),
